 ...
 * 2017-10-11
 */
public class ResponseDistrInReturnDetailVO implements Serializable {
    /**
     * 主键ID
     */
     ...
     * 行优惠（整单优惠分摊到行的金额）
     */
    @ApiModelProperty(value = "优惠（前端暂时不用）")
    private BigDecimal lineDiscountAmount;

    /**
     * 实际单价（实际金额/数量）
     */
    @ApiModelProperty(value = "实际单价（前端暂时不用）")
    private BigDecimal realPrice;

    /**
     * 实际金额
     */
    @ApiModelProperty(value = "实际金额（前端暂时不用）")
     ...
     * 数量
     */
    @ApiModelProperty(value = "可用数量,（用于修改时，判断输入退货数量的最大数量），注意：若是要货计划生成的配退单详情查看时则为要货数量")
    private BigDecimal usableQuantity;
    
    /**
     * 批号信息列表
     */
    @ApiModelProperty(value = "批号信息列表(配送类型为要货计划的时候合并的批号返回值)")
    List<DistrInLotNumVO> lotNums = new ArrayList<DistrInLotNumVO>();
