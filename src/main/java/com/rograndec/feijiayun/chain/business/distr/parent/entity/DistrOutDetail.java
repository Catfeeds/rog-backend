 ...
 * 2017-10-07
 */
public class DistrOutDetail implements Serializable {
    /**
     * 主键ID
     */
     ...
     * 单据（配货出库单）类型（5412）
     */
    @ApiModelProperty(value = "单据（配货出库单）类型（5412）")
    private Integer orderType;

    /**
     * 配货出库单单号
     */
    @ApiModelProperty(value = "配货出库单单号")
    ...
    public static List<DistrOutDetail> getDistrOutDetails(PickOrder pickOrder,DistrSend distrSend){
        List<PickDetail> pickDetails = pickOrder.getPickDetails();

        List<DistrOutDetail> distrOutDetails = new ArrayList<>();

        List<DistrSendDetail> distrSendDetails = distrSend.getDistrSendDetailList();
        ...
        for(DistrSendDetail distrSendDetail : distrSendDetails){
             ...
             * 企业ID
             */
            distrOutDetail.setEnterpriseId(distrSendDetail.getEnterpriseId());

            /**
             * 上级企业ID
             */
            distrOutDetail.setParentId(distrSendDetail.getParentId());


            /**
             * 基础单据明细ID
             */
            distrOutDetail.setBaseOrderDtlId(distrOutDetail.getId());
             ...
             * 基础单据日期
             */
            distrOutDetail.setBaseOrderDate(distrSend.getDistrDate());

            /**
             * 商品ID
             */
            distrOutDetail.setGoodsId(distrSendDetail.getGoodsId());

            /**
             * 商品编码
             */
            distrOutDetail.setGoodsCode(distrSendDetail.getGoodsCode());

            /**
             * 条形码
             */
            distrOutDetail.setBarcode(distrSendDetail.getBarcode());

            /**
             * 商品名称
             */
            distrOutDetail.setGoodsName(distrSendDetail.getGoodsName());

            /**
             * 商品通用名称
             */
            distrOutDetail.setGoodsGenericName(distrSendDetail.getGoodsGenericName());

            /**
             * 剂型ID
             */
            distrOutDetail.setDosageId(distrSendDetail.getDosageId());

            /**
             * 剂型名称
             */
            distrOutDetail.setDosageName(distrSendDetail.getDosageName());

            /**
             * 单位ID
             */
            distrOutDetail.setUnitId(distrSendDetail.getUnitId());

            /**
             * 单位名称
             */
            distrOutDetail.setUnitName(distrSendDetail.getUnitName());

            /**
             * 商品规格
             */
            distrOutDetail.setGoodsSpecification(distrSendDetail.getGoodsSpecification());

            /**
             * 生产厂商ID
             */
            distrOutDetail.setManufacturerId(distrSendDetail.getManufacturerId());

            /**
             * 生产厂商
             */
            distrOutDetail.setManufacturer(distrSendDetail.getManufacturer());

            /**
             * 商品产地
             */
            distrOutDetail.setGoodsPlace(distrSendDetail.getGoodsPlace());

            /**
             * 批准文号
             */
            distrOutDetail.setApprovalNumber(distrSendDetail.getApprovalNumber());

            /**
             * 数量
             */
            distrOutDetail.setQuantity(distrSendDetail.getQuantity());

            /**
             * 单价
             */
            distrOutDetail.setUnitPrice(distrSendDetail.getUnitPrice());

            /**
             * 行折扣（%）
             */
            distrOutDetail.setLineDiscount(distrSendDetail.getLineDiscount());

            /**
             * 金额（整单优惠前金额）
             */
            distrOutDetail.setAmount(distrSendDetail.getAmount());

            /**
             * 整单折扣（%）
             */
            distrOutDetail.setWholeDiscount(distrSendDetail.getWholeDiscount());

            /**
             * 行优惠（整单优惠分摊到行的金额）
             */
            distrOutDetail.setLineDiscountAmount(distrSendDetail.getLineDiscountAmount());

            /**
             * 实际单价（实际金额/数量）
             */
            distrOutDetail.setRealPrice(distrSendDetail.getRealPrice());

            /**
             * 实际金额
             */
            distrOutDetail.setRealAmount(distrSendDetail.getRealAmount());

            /**
             * 税率ID
             */
            distrOutDetail.setTaxRateId(distrSendDetail.getTaxRateId());

            /**
             * 税率
             */
            distrOutDetail.setTaxRate(distrSendDetail.getTaxRate());

            /**
             * 不含税实际单价
             */
            distrOutDetail.setNotaxRealPrice(distrSendDetail.getNotaxRealPrice());

            /**
             * 不含税实际金额
             */
            distrOutDetail.setNotaxRealAmount(distrSendDetail.getNotaxRealAmount());

            /**
             * 税额
             */
            distrOutDetail.setTaxAmount(distrSendDetail.getTaxAmount());


            /**
             * 行号
             */
            distrOutDetail.setLineNum(i);

            for(PickDetail pickDetail : pickDetails){
                if(pickDetail.getGoodsId().equals(distrSendDetail.getGoodsId())){
                    List<DistrOutShelf> distrOutShelfs = DistrOutShelf.getDistrOutShelfs(pickDetail, distrSendDetail);
                    distrOutDetail.setDistrOutShelfList(distrOutShelfs);
                }

            }


            distrOutDetails.add(distrOutDetail);
        }

        return distrOutDetails;
    }

    @ApiModelProperty(value = "出库单货位信息")
    private List<DistrOutShelf> distrOutShelfList = new ArrayList<>();

    public List<DistrOutShelf> getDistrOutShelfList() {
        return distrOutShelfList;
    }

    public void setDistrOutShelfList(List<DistrOutShelf> distrOutShelfList) {
        this.distrOutShelfList = distrOutShelfList;
    }
    ...
    /**
     * 单据（配货出库单）类型（5412）
     * @return order_type 单据（配货出库单）类型（5412）
     */
    ...
    /**
     * 单据（配货出库单）类型（5412）
     * @param orderType 单据（配货出库单）类型（5412）
     */
