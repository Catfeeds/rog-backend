 ...
 * 2017-10-08
 */
public class DistrReturnInDetail implements Serializable {
    /**
     * 主键ID
     */
    ...
    public static DistrReturnInDetail afterGenerateReturnInSetdetail(DistrReturnInDetail distrReturnInDetail,DistrReturnIn distrReturnIn){
        /**
         * 整单折扣（%）
         */
        distrReturnInDetail.setWholeDiscount(distrReturnIn.getWholeDiscount());
         ...
         * @throws
         */
        BigDecimal lineDiscountAmount = CalculateComponent.getLineRoundOffByLineAmountAndWholeAmountTotal(
                distrReturnIn.getWholeDiscountAmount()
                , distrReturnInDetail.getAmount()
                , distrReturnIn.getAmountTotal()
        );
        distrReturnInDetail.setLineDiscountAmount(lineDiscountAmount);
        ...
        BigDecimal realAmount = CalculateComponent.getRealAmountByQuantityAndPriceAndLineDiscount(
                distrReturnInDetail.getQuantity()
                , distrReturnInDetail.getUnitPrice()
                , distrReturnInDetail.getLineDiscount()
                , distrReturnIn.getWholeDiscount()
                , distrReturnInDetail.getLineDiscountAmount()
        );
        distrReturnInDetail.setRealAmount(realAmount);
        ...
        BigDecimal realPrice = CalculateComponent.getRealPriceByRealAmountAndQuantity(
                distrReturnInDetail.getRealAmount()
                , distrReturnInDetail.getQuantity()
        );
        distrReturnInDetail.setRealPrice(realPrice);
        ...
        BigDecimal notaxRealAmount = CalculateComponent.getNotaxAmountByRealAmountAndTaxRate(
                distrReturnInDetail.getRealAmount()
                , distrReturnInDetail.getTaxRate()
        );
        distrReturnInDetail.setNotaxRealAmount(notaxRealAmount);
        ...
        BigDecimal notaxRealPrice = CalculateComponent.getNotaxPriceByNotaxAmountAndQuantity(
                distrReturnInDetail.getNotaxRealAmount()
                , distrReturnInDetail.getQuantity()
        );
        distrReturnInDetail.setNotaxRealPrice(notaxRealPrice);
        ...
        distrReturnInDetail.setTaxAmount(taxAmount);

        return distrReturnInDetail;
    }


    public static DistrReturnInDetail afterSaveDistrReturnInSetDetail(DistrReturnInDetail distrReturnInDetail,DistrReturnIn distrReturnIn){
        /**
         * 配后退回入库单ID
         */
        distrReturnInDetail.setReturnInId(distrReturnIn.getId());

        /**
         * 单据（配后退回入库单）类型（5425）
         */
        distrReturnInDetail.setOrderType(distrReturnIn.getOrderType());

        /**
         * 配进入库单号
         */
        distrReturnInDetail.setReturnInCode(distrReturnIn.getCode());

        /**
         * 入库日期
         */
        distrReturnInDetail.setReturnInDate(distrReturnIn.getReturnInDate());

        /**
         * 基础单据ID
         */
        distrReturnInDetail.setBaseOrderId(distrReturnIn.getBaseOrderId());
        /**
         * 基础单据类型
         */
        distrReturnInDetail.setBaseOrderType(distrReturnIn.getBaseOrderType());

        /**
         * 基础单据编码
         */
        distrReturnInDetail.setBaseOrderCode(distrReturnIn.getBaseOrderCode());

        /**
         * 基础单据日期
         */
        distrReturnInDetail.setBaseOrderDate(distrReturnIn.getBaseOrderDate());

        /**
         * 配进订单ID
         */
        distrReturnInDetail.setDistrOrderId(distrReturnIn.getDistrOrderId());
    ...
    public static DistrReturnInDetail getDistrReturnInDetail(DistrReturnInDetailParam distrReturnInDetailParam) throws Exception {

        DistrReturnInDetail distrReturnInDetail = new DistrReturnInDetail();

        UserVO userVO = distrReturnInDetailParam.getUserVO();

        DistrReturnInDetailFormVO distrReturnInFormVO = distrReturnInDetailParam.getDistrReturnInFormVO();

        List<DistrReturnCheckDetail> distrReturnCheckDetails = distrReturnInDetailParam.getDistrReturnCheckDetails();

        List<GoodsTaxRate> goodsTaxRates = distrReturnInDetailParam.getGoodsTaxRates();

        /**
         * 企业ID
         */
        distrReturnInDetail.setEnterpriseId(userVO.getEnterpriseId());
        if(userVO.getChainType().equals(ChainType.Headquarters.getCode())){
            distrReturnInDetail.setParentId(0L);
        }else {
            distrReturnInDetail.setParentId(userVO.getParentId());
        }

        /**
         * 基础单据明细ID
         */
        distrReturnInDetail.setBaseOrderDtlId(distrReturnInFormVO.getCheckDtId());
         ...
         * 单价
         */
        distrReturnInDetail.setUnitPrice(distrReturnInFormVO.getUnitPrice());

        /**
         * 行折扣（%）
         */
        distrReturnInDetail.setLineDiscount(distrReturnInFormVO.getLineDiscount());





        goodsTaxRates.forEach(gtr -> {
            if(gtr.getId().equals(distrReturnInFormVO.getTaxRateId())){
                /**
                 * 税率ID
                 */
                distrReturnInDetail.setTaxRateId(gtr.getId());
                /**
                 * 税率
                 */
                distrReturnInDetail.setTaxRate(gtr.getTaxRate());
            }
        });
        ...
        distrReturnCheckDetails.forEach(drcd -> {
            if(drcd.getId().equals(distrReturnInFormVO.getCheckDtId())){

                /**
                 * 配进订单明细ID
                 */
                distrReturnInDetail.setDistrOrderDtlId(drcd.getNoticeDtlId());

                /**
                 * 商品ID
                 */
                distrReturnInDetail.setGoodsId(drcd.getGoodsId());

                /**
                 * 商品编码
                 */
                distrReturnInDetail.setGoodsCode(drcd.getGoodsCode());

                /**
                 * 条形码
                 */
                distrReturnInDetail.setBarcode(drcd.getBarcode());

                /**
                 * 商品名称
                 */
                distrReturnInDetail.setGoodsName(drcd.getGoodsName());

                /**
                 * 商品通用名称
                 */
                distrReturnInDetail.setGoodsGenericName(drcd.getGoodsGenericName());

                /**
                 * 剂型ID
                 */
                distrReturnInDetail.setDosageId(drcd.getDosageId());

                /**
                 * 剂型名称
                 */
                distrReturnInDetail.setDosageName(drcd.getDosageName());

                /**
                 * 单位ID
                 */
                distrReturnInDetail.setUnitId(drcd.getUnitId());

                /**
                 * 单位名称
                 */
                distrReturnInDetail.setUnitName(drcd.getUnitName());

                /**
                 * 商品规格
                 */
                distrReturnInDetail.setGoodsSpecification(drcd.getGoodsSpecification());

                /**
                 * 生产厂商ID
                 */
                distrReturnInDetail.setManufacturerId(drcd.getManufacturerId());

                /**
                 * 生产厂商
                 */
                distrReturnInDetail.setManufacturer(drcd.getManufacturer());

                /**
                 * 商品产地
                 */
                distrReturnInDetail.setGoodsPlace(drcd.getGoodsPlace());

                /**
                 * 批准文号
                 */
                distrReturnInDetail.setApprovalNumber(drcd.getApprovalNumber());
                /**.
                 *
                 * 行金额（整单优惠前金额）
                 */
                BigDecimal amount = DistrReturnInDetail.calcLineDiscount(distrReturnInFormVO, drcd.getReceiveQuantity());
                /**
                 * 金额（整单优惠前金额）
                 */
                distrReturnInDetail.setAmount(amount.setScale(2,BigDecimal.ROUND_HALF_UP));

                /**
                 * 数量
                 */
                distrReturnInDetail.setQuantity(drcd.getReceiveQuantity());

                /**
                 * 未清数量
                 */
                distrReturnInDetail.setUnclearQuantity(drcd.getReceiveQuantity());
            }
        });
         ...
         * 已清数量
         */
        distrReturnInDetail.setClearQuantity(new BigDecimal(0));

        /**
         * 行号
         */
        distrReturnInDetail.setLineNum(distrReturnInFormVO.getLineNum());

        /**
         * 状态
         */
        distrReturnInDetail.setStatus(DistrReturnStatus.WAIT_BILL);

        /**
         * 备注
         */
        distrReturnInDetail.setRemark(distrReturnInFormVO.getRemark());

        UserEnterpriseUtils.setUserCreateOrModify(distrReturnInDetail,userVO,true);
     ...
     * @return
     */
    public static BigDecimal calcLineDiscount(DistrReturnInDetailFormVO detailVO,BigDecimal quantity){
    ...
    /**
     * 单据（配后退回入库单）类型（5425）
     * @return order_type 单据（配后退回入库单）类型（5425）
     */
    ...
    /**
     * 单据（配后退回入库单）类型（5425）
     * @param orderType 单据（配后退回入库单）类型（5425）
     */
    ...
    /**
     * 配进入库单号
     * @param returnInCode 配进入库单号
     */
    public void setReturnInCode(String returnInCode) {
        this.returnInCode = returnInCode == null ? null : returnInCode.trim();
    }
    ...
    /**
     * 配进订单明细ID
     * @return distr_order_dtl_id 配进订单明细ID
     */
    ...
    /**
     * 配进订单明细ID
     * @param distrOrderDtlId 配进订单明细ID
     */
    ...
    public String toString() {
        ...
        sb.append(", returnInId=").append(returnInId);
        sb.append(", orderType=").append(orderType);
        sb.append(", returnInCode=").append(returnInCode);
        ...
        sb.append(", distrOrderDtlId=").append(distrOrderDtlId);
