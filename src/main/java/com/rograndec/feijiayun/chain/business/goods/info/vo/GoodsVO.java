...
public class GoodsVO implements Serializable{
     ...
     * 标准库ID
     */
    @ApiModelProperty(value="MPH库商品ID")
    ...
    private String pinyinCodeBegin;

    private String pinyinCodeEnd;
     ...
     */
    @ApiModelProperty(value="配货标识（0-普通；1-首推），默认为0-普通")
    ...
    @ApiModelProperty(value="审核状态（0-待审核；1-审核通过；2-审核拒绝；-2-审核撤回）  ")
   ...
   //商品资质信息
    ...
    public String getPinyinCodeBegin() {
        if(pinyinCodeBegin == null){
            if(pinyinCode != null){
                String[] split = pinyinCode.split("-");
                if(split.length >= 1){
                    return split[0];
                } else {
                    return "";
        ...
        return pinyinCodeBegin;
    }

    public void setPinyinCodeBegin(String pinyinCodeBegin) {
        this.pinyinCodeBegin = pinyinCodeBegin;
    }

    public String getPinyinCodeEnd() {
        if(pinyinCodeEnd == null){
            if(pinyinCode != null){
                String[] split = pinyinCode.split("-");
                if(split.length >= 2){
                    return split[1];
        ...
        return pinyinCodeEnd;
    }

    public void setPinyinCodeEnd(String pinyinCodeEnd) {
        this.pinyinCodeEnd = pinyinCodeEnd;
    }
