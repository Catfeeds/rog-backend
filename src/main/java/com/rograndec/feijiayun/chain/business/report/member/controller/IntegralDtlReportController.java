 ...
 *
 */
@Api(value = "report_member_integralDtl",description = "报表-零售管理-积分明细")
@RestController
@RequestMapping("report/member/integralDtl")
@Validated
public class IntegralDtlReportController {
	
	private static final Logger logger = LoggerFactory.getLogger(IntegralDtlReportController.class);
	...
	@ApiOperation(value = "查询", notes = "查询积分明细列表 | 开发者 yuting.li | 已联调")
	@ResponseBody
    @RequestMapping(value="/queryIntegralDtlList", method=RequestMethod.POST)
    public Result<Page<List<MemberIntegralDtlVO>>> queryIntegralDtlList(HttpSession session,@Valid @RequestBody MemberIntegralExchangeParamVO param){
		Result<Page<List<MemberIntegralDtlVO>>> result = new Result<>();
		...
		try {
            ...
            Page<List<MemberIntegralDtlVO>> page = memberReportService.getIntegralDtl(param);
            result.setBizCodeSuccessInfo(SysCode.SUCCESS, page);
		} catch (Exception e) {
			logger.error("获取积分明细数据失败:", e);
	...
	@ApiOperation(value = "导出Excel", notes = "按照模版积分明细列表导出至Excel | 开发者 yuting.li | 已联调")
    ...
    public void exportExcel(HttpSession session,HttpServletResponse response,MemberIntegralExchangeParamVO param){
        ...
        try {
	        ...
	        memberReportService.exportIntegralDtl(output, param);
            output.close();
            output.flush();
        } catch (Exception e) {
        	logger.error("导出积分明细错误:",e);
        }
	}
