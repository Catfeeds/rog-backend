 ...
 *
 */
@Api(value = "report_member_storedValuePayment",description = "报表-零售管理-储值付款")
@RestController
@RequestMapping("report/member/storedValuePayment")
@Validated
public class StoredValuePaymentReportController {
	
	private static final Logger logger = LoggerFactory.getLogger(StoredValuePaymentReportController.class);
	...
	@ApiOperation(value = "查询储值付款列表", notes = "查询储值付款列表 | 开发者 yuting.li | 已联调")
	@ResponseBody
    @RequestMapping(value="/queryStoredValuePaymentList", method=RequestMethod.POST)
    public Result<Page<MemberAmountRecordTotalVO>> queryStoredValuePaymentList(HttpSession session,@Valid @RequestBody MemberIntegralExchangeParamVO param){
		Result<Page<MemberAmountRecordTotalVO>> result = new Result<>();
		...
		try {
			...
			Page<MemberAmountRecordTotalVO> page = memberReportService.getStoredAmount(param);
			result.setBizCodeSuccessInfo(SysCode.SUCCESS, page);
		} catch (Exception e) {
			logger.error("查询储值付款数据失败:", e);
	...
	@ApiOperation(value = "导出Excel", notes = "储值付款导出至Excel | 开发者 yuting.li | 已联调")
    ...
    public void exportExcel(HttpSession session,HttpServletResponse response,MemberIntegralExchangeParamVO param){
        ...
        try {
	        ...
	        memberReportService.exportMemberAmountRecord(output, param);
            output.close();
            output.flush();
        } catch (Exception e) {
        	logger.error("导出储值付款错误:",e);
        }
	}
