...
@Api(value = "report_quality_equipment_equipmentCheck",description = "报表-质量管理-设施与设备-校准或检定")
@RestController
@RequestMapping("/report/quality/equipment/equipmentCheck")
@Validated
public class EquipmentCheckReportController {
	private static final Logger logger = LoggerFactory.getLogger(EquipmentStandAccountReportController.class);

	@Autowired
	private EquipmentCheckService equipmentService;

	@ApiOperation(value = "校准或检定报表", notes = "获取数据 | 开发者 zhengbin.jin | 开发完成", produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	@RequestMapping(value = "/getEquipmentReportByParam", method = RequestMethod.POST)
	public Result<Page<List<EquipmentCheck>>> getEquipmentReportByParam(HttpSession session,
	...
	@ApiOperation(value = "校准或检定报表-获取打印数据", notes = "获取数据 | 开发者 zhengbin.jin | 开发完成", produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	...
	public Result<List<EquipmentCheck>> getEquipmentPrintByParam(HttpSession session,
															RequestEquipmentListEVO requestEquipmentListEVO) throws Exception {
		Result<List<EquipmentCheck>> result = new Result<>();
		try {
			UserVO userVO = (UserVO) session.getAttribute("user");
			List<EquipmentCheck> equipmentList = equipmentService.listEquipmentPrintData(userVO, requestEquipmentListEVO);
	...
	@ApiOperation(value = "校准或检定报表-导出数据", notes = "获取数据 | 开发者 zhengbin.jin | 开发完成")
	@ResponseBody
