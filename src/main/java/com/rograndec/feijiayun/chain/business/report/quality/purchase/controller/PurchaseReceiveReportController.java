 ...
 *
 */
@Api(value = "report_quality_purchase_purchaseReceive",description = "报表-质量管理-收货与验收-采购收货")
@RestController
@RequestMapping("report/quality/purchase/purchaseReceive")
@Validated
public class PurchaseReceiveReportController {

    private static final Log logger = LogFactory.getLog(PurchaseReceiveReportController.class);

    @Autowired
    PurchaseReceiveReportService purchaseReceiveReportService;

    @ApiOperation(value = "获取采购收货列表", notes = "获取采购收货列表 | 开发者:马东 | 已完成" )
    ...
    public Result<Page<TotalReportVO<PurchaseReceiveReportVO>>> getPurchaseReceive(HttpServletRequest request,
        ...
        try {
            ...
            logger.error("获取采购收货列表错误:" + e.getMessage(), e);
            result.setBizCodeFallInfo(SysCode.FAIL);
            return result;
        }
        return result;
    }
    @ApiOperation(value = "导出Excel", notes = "按照模版将采购收货列表信息导出至Excel | 开发者:马东 | 已完成")
    @RequestMapping(value="/exportExcel", method=RequestMethod.POST, produces = MediaType.APPLICATION_FORM_URLENCODED_VALUE)
    public void exportExcel(HttpServletRequest request, HttpServletResponse response, ReceivePageVO receivePageVO){
        ...
        try {
            ...
            logger.error("导出采购收货信息错误:"+e.getMessage(),e);
        }
    }
