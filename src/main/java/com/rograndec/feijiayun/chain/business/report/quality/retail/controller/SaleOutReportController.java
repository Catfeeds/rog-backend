 ...
 *
 */
@Api(value = "report_quality_retail_saleOut",description = "报表-质量管理-销售-销售出库")
@RestController
@RequestMapping("report/quality/retail/saleOut")
@Validated
public class SaleOutReportController {

	private static final Logger logger = LoggerFactory.getLogger(SaleOutReportController.class);
	
	@Autowired
	private SaleOutORreturnService saleOutORreturnService;
	
	@ApiOperation(value = "查询", notes = "查询销售出库列表 | 开发者 yuting.li | 已联调",produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
    @RequestMapping(value="/getSaleOutList", method=RequestMethod.POST)
    public Result<Page<SaleOutORreturnTotalVO>> getSaleOutList(HttpSession session,@Valid @RequestBody SaleOutORreturnParamVO paramVO){
	...
	@ApiOperation(value = "导出Excel", notes = "销售出库列表导出至Excel | 开发者 yuting.li | 已联调")
