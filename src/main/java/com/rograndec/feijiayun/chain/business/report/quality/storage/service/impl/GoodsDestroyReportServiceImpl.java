 ...
 */
@Service
public class GoodsDestroyReportServiceImpl implements GoodsDestroyReportService {
    @Autowired
    ...
    @Override
    public Page getGoodsDestroyReportList(RequestReportGoodsDestroyVO requestVO, Page page, UserVO userVO) throws Exception {
        ...
        if (sortDate != null && sortDate == 0) {
            sort += "gd.destroy_date,";
        }
        if (sortDate != null && sortDate == 1) {
            sort += "gd.destroy_date desc,";
        }
        ...
        BaseGoodsReportTotalVO<GoodsDestroyReportVO> goodsDestroyReportTotalVO = new BaseGoodsReportTotalVO<>();
        //获取总数量
        BigDecimal quantityTotal = goodsSeriesReportMapper.getGoodsDestroyReportQuantityTotal(requestVO);
        goodsDestroyReportTotalVO.setQuantityTotal(quantityTotal == null?BigDecimal.ZERO:quantityTotal);
        ...
        List<GoodsDestroyReportVO> goodsDestroyReportVOList = goodsSeriesReportMapper.getGoodsDestroyReport(requestVO);
        goodsDestroyReportTotalVO.setGoodsReportList(goodsDestroyReportVOList == null ? new ArrayList<>() : goodsDestroyReportVOList);

        page.setResult(goodsDestroyReportTotalVO);
        if (hPage != null) {
    ...
    @Override
    public void excelExport(OutputStream output, BaseGoodsReportTotalVO<GoodsDestroyReportVO> goodsDestroyReportTotalVO, UserVO userVO) {
        //标题数据
        ...
        map.put("destroyDate", "日期");
        map.put("destroyCode", "单号");
        map.put("destroyManName", "销毁人员");
        map.put("destroyReasonDesc", "销毁原因");
        map.put("destroyTime", "销毁时间");
        map.put("destroyMode", "销毁方式");
        map.put("destroyUnit", "销毁机构");
        map.put("destroyPlace", "销毁地点");
        map.put("monitor", "监督人员1");
        map.put("secondMonitor", "监督人员2");
        ...
        purchaseGeneralComponent.commExcelExport(output, map, goodsDestroyReportTotalVO.getGoodsReportList(),
                names, null, goodsDestroyReportTotalVO.getQuantityTotal().toString(), false, needTotalName);
    }
}
