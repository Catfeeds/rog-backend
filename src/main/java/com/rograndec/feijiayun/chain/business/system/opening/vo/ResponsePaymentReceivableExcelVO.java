 ...
 * 期初应付/应收导入返回的信息实体
 */
public class ResponsePaymentReceivableExcelVO implements Serializable {

    @ApiModelProperty(value = "导入时存储数据的key值，在导入成功数据或者导出合格/不合格商品信息时传递该值获取对应的数据")
    private String key;
    //合格数据
    private List<OpeningPaymentReceivableExcelVO> qualifiedList = new ArrayList<>();
    //不合格数据
    private List<OpeningPaymentReceivableExcelVO> disqualificationList = new ArrayList<>();
