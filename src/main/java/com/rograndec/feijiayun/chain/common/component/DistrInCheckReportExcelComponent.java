 ...
 */
@Component
public class DistrInCheckReportExcelComponent<T> {
	            ...
	            ExcelWriter writer = new ExcelWriter() {
	                public void generate() throws Exception {
                        ...
                        if(user.getChainType()!=0){
                        	 ...
                        	 this.createCell(23,quantity.get("receiveQuantity")+"");
                             this.createCell(25,quantity.get("samplingQuantity")+"");
                             this.createCell(26,quantity.get("qualifiedQuantity")+"");
                             this.createCell(28,quantity.get("unqualifiedQuantity")+"");
                        }
                        // 结束行
                        this.endRow(); 
	                    // 电子表格结束
	 ...
	 private void createRowHeader(ExcelWriter writer,UserVO user,LinkedHashMap<String, String> rowHeaderList) throws IOException{
		 ...
		 if(user.getChainType()!=0){
			 ...
			 writer.createCell(21,user.getEnterpriseName());
		 }
		 writer.endRow();
		 //插入第二行
		 writer.insertRow(1);
		 if(user.getChainType()!=0){
			 writer.createCell(20, "配进验收单");
		 }else{
			 writer.createCell(21, "配进验收单");
		 }
		 writer.endRow();
		 int cellNum=0;
		 //插入第三行
