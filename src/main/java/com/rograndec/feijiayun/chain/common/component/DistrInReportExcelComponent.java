 ...
 */
@Component
public class DistrInReportExcelComponent<T> {
	        ...
	        try {
	            ExcelWriter writer = new ExcelWriter() {
	                public void generate() throws Exception {
                        ...
                        this.createCell(25,quantity.get("quantity")+"");
	 ...
	 private void createRowHeader(ExcelWriter writer,UserVO user,LinkedHashMap<String, String> rowHeaderList) throws IOException{
		 //插入第一行
		 writer.insertRow(0);
		 writer.createCell(25,user.getEnterpriseName());
		 writer.endRow();
		 //插入第二行
		 writer.insertRow(1);
		 writer.createCell(25, "配进入库单");
		 writer.endRow();
		 int cellNum=0;
		 //插入第三行
		 writer.insertRow(2);
