 ...
 */
@Component
public class DistrReturnInReportExcelComponent<T> {
	        ...
	        try {
	            ExcelWriter writer = new ExcelWriter() {
	                public void generate() throws Exception {
                        ...
                        this.createCell(24,quantity.get("quantity")+"");
                        this.createCell(27,quantity.get("amount")+"");
                        this.createCell(31,quantity.get("realAmount")+"");
                        this.createCell(34,quantity.get("notaxRealAmount")+"");
                        this.createCell(35,quantity.get("taxAmount")+"");
                        // 结束行
                        this.endRow(); 
	                    // 电子表格结束
	                    this.endSheet();
	 ...
	 private void createRowHeader(ExcelWriter writer,UserVO user,LinkedHashMap<String, String> rowHeaderList) throws IOException{
		 //插入第一行
		 writer.insertRow(0);
		 writer.createCell(24,user.getEnterpriseName());
		 writer.endRow();
		 //插入第二行
		 writer.insertRow(1);
		 writer.createCell(24, "配后退回入库单");
		 writer.endRow();
		 int cellNum=0;
		 //插入第三行
		 writer.insertRow(2);
