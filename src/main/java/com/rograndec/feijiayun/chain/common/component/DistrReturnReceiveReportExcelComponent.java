 ...
 */
@Component
public class DistrReturnReceiveReportExcelComponent<T> {
	        ...
	        try {
	            ExcelWriter writer = new ExcelWriter() {
	                public void generate() throws Exception {
                        ...
                        this.createCell(18,quantity.get("quantity")+"");
                        this.createCell(19,quantity.get("arrivalQuantity")+"");
                        this.createCell(20,quantity.get("receiveQuantity")+"");
                        this.createCell(21,quantity.get("refuseQuantity")+"");
                        // 结束行
                        this.endRow(); 
	                    // 电子表格结束
	                    this.endSheet();
	 ...
	 private void createRowHeader(ExcelWriter writer,UserVO user,LinkedHashMap<String, String> rowHeaderList) throws IOException{
		 //插入第一行
		 writer.insertRow(0);
		 writer.createCell(17,user.getEnterpriseName());
		 writer.endRow();
		 //插入第二行
		 writer.insertRow(1);
		 writer.createCell(17, "配后退回收货单");
		 writer.endRow();
		 int cellNum=0;
		 //插入第三行
		 writer.insertRow(2);
