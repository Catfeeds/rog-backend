 ...
 */
@Component
public class NearEffectGoodsExcelComponent<T> {
	
	
	
	 public void commExcelExport(OutputStream output,LinkedHashMap<String, String> rowHeaderList, List<T> ts,UserVO user,BigDecimal quantity){
	        try {
	            ExcelWriter writer = new ExcelWriter() {
	                public void generate() throws Exception {
                        ...
                        if(user.getChainType()==0){
                        	 this.createCell(18,quantity+"");
                        }else{
                        	this.createCell(16,quantity+"");
                        }
                        // 结束行
                        this.endRow(); 
	                    // 电子表格结束
	 ...
	 private void createRowHeader(ExcelWriter writer,UserVO user,LinkedHashMap<String, String> rowHeaderList) throws IOException{
		 ...
		 writer.insertRow(0);
		 if(chainType==0){
			 writer.createCell(16,user.getEnterpriseName());
		 }else{
			 writer.createCell(15,user.getEnterpriseName());
		 }
		 writer.endRow();
		 //插入第二行
		 writer.insertRow(1);
		 if(chainType==0){
			 writer.createCell(16, "近效期商品");
		 }else{
			 writer.createCell(15, "近效期商品");
		 }
		 writer.endRow();
		 int cellNum=0;
		 //插入第三行
