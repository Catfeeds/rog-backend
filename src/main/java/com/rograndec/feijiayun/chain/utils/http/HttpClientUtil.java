package com.rograndec.feijiayun.chain.utils.http;

import com.alibaba.fastjson.JSONObject;
import com.rograndec.feijiayun.chain.exception.BusinessException;
import com.rograndec.feijiayun.chain.inf.rogrand.utils.HttpsUtil;
...
public class HttpClientUtil {
	...
	public static String sendGetRequestJson(String reqURL, String apikey, String decodeCharset) {
	 ...
	 *            请求地址
	 * @param sendData
	 *            请求参数,若有多个参数则应拼接成param11=value11&param22=value22&param33=value33的形式后,传入该参数中
	 * @param isEncoder
	 ...
	 * 发送HTTPS_POST请求
	 *
	 * @see 该方法为<code>sendHttpsPostRequest(String,String,boolean,String,String)</code>的简化方法
	 * @see 该方法在对请求数据的编码和响应数据的解码时,所采用的字符集均为UTF-8
	 * @see 当<code>isEncoder=true</code>时,其会自动对<code>sendData</code>中的[中文][|][
	 *      ]等特殊字符进行<code>URLEncoder.encode(string,"UTF-8")</code>
	 * @param isEncoder
	 ...
	 * @author jian.mei
	 */
	public static String sendHttpsPostRequest(String reqURL, String sendData, boolean isEncoder) {
		return sendHttpsPostRequest(reqURL, sendData, isEncoder, null, null);
	}


	public static JSONObject sendRGTHttpsPostRequest(String reqURL, String sendData, boolean isEncoder) {
		logger.info("请求URL={}{}",reqURL,sendData);
		JSONObject jsonObject = JSONObject.parseObject(sendHttpsPostRequest(reqURL, sendData, isEncoder, null, null));
		if(jsonObject == null){
			throw new BusinessException("111111","融贯通接口返回异常");
		}
		return jsonObject.getJSONObject("body");
	}
	public static String sendHttpsPostRequest(String reqURL, String sendData, boolean isEncoder, String encodeCharset,
										 String decodeCharset) {
		String responseContent = null;

		HttpClientBuilder httpClientBuilder = HttpClientBuilder.create();

		HttpsUtil.configureHttpClient2(httpClientBuilder);
...
//		HttpClient httpClient = new DefaultHttpClient();
	...
	public static String getIpArea(String ip) {
		...
		try {
			...
			inStream.close();
			JSONObject jsonObject = (JSONObject) JSONObject.parseObject(info).get("data");
			String city = StringEscapeUtils.escapeSql(jsonObject.getString("city"));
			...
			logger.error("根据ip=[ " + ip + "]查看所属城市异常",e.getMessage());
			return "";
		} finally {
			try {
				if (inStream != null){
